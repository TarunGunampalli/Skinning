...
export class GUI implements IGUI {
	...
	public drag(mouse: MouseEvent): void {
		...
		scene.meshes.forEach((mesh) => {
			mesh.bones.forEach((bone) => {
				if (this.boneIntersect(bone, mouseRay).intersect) console.log(bone);
	...
	private boneIntersect(bone: Bone, ray: Ray): Intersection {
		...
		// console.log(R.all());
		if (p0.y < 0 && p0.y > b && p1.y < 0 && p1.y > b) return { intersect: false };
		else if (p1.y < 0 && p1.y > b) return { intersect: true, t0 };
		else if (p0.y < 0 && p0.y > b) return { intersect: true, t0: t1 };
